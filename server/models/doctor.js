...
function DoctorModel(){
	...
	this.create_doctor = function(doctor, callback) {
		...
		connection.query('SELECT id FROM doctors WHERE email = ?', [doctor.email], function(error, result) {
			...
			connection.query("INSERT INTO doctors SET first_name = ?, middle_name = ?, last_name = ?, sex = ?, date_of_birth = ?, mobile = ?, type = ?, email = ?, email_verified = false, certified = false, password = ?, level = 1, created_at = NOW(), updated_at = NOW()", data, function(err, result) {
				if(err){
					console.log(err);
					callback({error: true, errors: err});
				}
	...
	this.login_doctor =function(doctor, callback) {
		...
		connection.query("SELECT * FROM doctors WHERE email = ?", [doctor.email], function (error, result) {
			...
			jwt.sign({ id: result[0].id, email: result[0].email, level: result[0].level, iat: Math.floor(Date.now() / 1000) - 30 }, cert, { expiresIn: '2h' }, function(err, token) {
				callback({error: false, data: result, token: token});
			});
		});
	};

	this.get_careType = function(callback) {
		connection.query("SELECT * FROM doctors", function (err, result) {
			if(err){
				callback({error: true, errors: err});
			}
	...
	this.get_careType = function(id, callback) {
		connection.query("SELECT * FROM doctors WHERE id = ?", [id], function (err, result) {
			if(err){
				callback({error: true, errors: err});
			}
	...
	this.create_careType = function(doctor, callback) {
		var err = {};
	};

	this.update_careType = function(id, doctor, callback) {
		var err = {};
	};

	this.delete_careType = function(id, callback) {
		connection.query("DELETE FROM doctors WHERE id = ?", [id], function (err, result) {
			if(err){
				callback({error: true, errors: err});
			}
